"""Tareas Celery para el conector Shopify (Fase 18).
...
@celery_app.task(
    name="connectors.shopify.embed_product",
    bind=True,
    max_retries=3,
    default_retry_delay=30,
)
def embed_product(self, product_id: str, tenant_id: str, config_id: str) -> dict:
    """Genera y persiste el embedding semántico de un producto Shopify.
    ...
    if not api_key:
        logger.warning("embed_product (shopify): VOYAGE_API_KEY no configurado, saltando.")
    ...
    try:
        with get_db_session() as db:
            ...
            if product is None:
                logger.warning("embed_product (shopify): producto %s no encontrado.", product_id)
        ...
        logger.info("embed_product (shopify): producto %s embebido OK.", product_id)
        return {"ok": True}

    except Exception as exc:
        logger.error("embed_product (shopify) error para %s: %s", product_id, exc)
